...
void loop(){
  if(connected){
    RTLap LapInfo; //create a struct to save all the data
    LapInfo = acTelemetry.readSpot(); //save on the new struct all the data returned by readUpdate
    Serial.println(LapInfo.time); //write the value of the lap time on the serial
    //You can also access only one information by doing:
    //Serial.println(acTelemetry.readSpot().time);
  }
  delay(10);
}
